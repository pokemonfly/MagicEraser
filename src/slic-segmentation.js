 ...
 * LongLong Yu 2014.
 */
// (function() { Convert RGBA into XYZ color space. rgba: Red Green Blue Alpha.
...
function computeSLICSegmentation( imageData, options ) {
    var imWidth = imageData.width,
        ...
        labData = xyz2lab( rgb2xyz( imageData.data, imageData.width, imageData.height ), imageData.width, imageData.height );
    // Compute edge.
    computeEdge( labData, edgeMap, imageData.width, imageData.height );
    // Initialize K-Means Centers.
    initializeKmeansCenters( labData, edgeMap, currentCenters, clusterParams, numRegionsX, numRegionsY, regionSize, imageData.width, imageData.height );
    ...
    for ( var iter = 0; iter < maxNumIterations; ++iter ) {
        // Do assignment.
        assignSuperpixelLabel( labData, segmentation, mcMap, msMap, distanceMap, currentCenters, clusterParams, numRegionsX, numRegionsY, regionSize, imageData.width, imageData.height );
        ...
        for ( i = 0; i < newCenters.length; ++i )
            newCenters[i] = 0;
        computeCenters( labData, segmentation, masses, newCenters, numRegions, imageData.width, imageData.height );
    ...
    //console.log(currentCenters); // compute centers. console.log(labData);   // log lab data console.log(edgeMap); // log edge map
    eliminateSmallRegions( segmentation, options.minRegionSize, numPixels, imageData.width, imageData.height );
    return segmentation;
}
...
function computeSegmentation( imageData, options ) {
    ...
    if ( options.callback ) {
        ...
        options.callback({ width: imageData.width, height: imageData.height, size: numSegments, indexMap: segmentation, rgbData: rgbData });
    }
}

// When image is loaded.
function onSuccessImageLoad( image, options ) {
    var segmentation = computeSegmentation( image, options );
}
...
export default SLICSegmentation
// }).call(this);
